...
describe("isInAppBrowser", () => {
  it("detects a variety of in-app browsers", () => {
    const samples: Array<[string, string]> = [
      ...
      ["Snapchat", "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) Snapchat"],
      ["WhatsApp", "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) WhatsApp"],
      ["KakaoStory", "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) kakaostory"],
    ...
    expect(isInAppBrowser("Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) Safari")).toBe(false);
  });
...
describe("getEscapeUrl", () => {
  ...
  it("adds LINE openExternalBrowser query param", () => {
    expect(getEscapeUrl(HTTPS_URL, LINE_ANDROID_UA)).toBe("https://example.com/path?foo=1&openExternalBrowser=1");
  ...
  it("uses x-safari-https for all iOS versions including iOS 16", () => {
    expect(getEscapeUrl(HTTPS_URL, IOS_15_UA)).toBe("x-safari-https://example.com/path?foo=1");
    expect(getEscapeUrl(HTTPS_URL, IOS_16_UA)).toBe("x-safari-https://example.com/path?foo=1");
    expect(getEscapeUrl(HTTPS_URL, IOS_17_UA)).toBe("x-safari-https://example.com/path?foo=1");
    expect(getEscapeUrl(HTTPS_URL, IOS_18_UA)).toBe("x-safari-https://example.com/path?foo=1");
  });

  it("uses x-safari-http for all iOS versions including iOS 16", () => {
    expect(getEscapeUrl(HTTP_URL, IOS_15_UA)).toBe("x-safari-http://example.com/path?foo=1");
    expect(getEscapeUrl(HTTP_URL, IOS_16_UA)).toBe("x-safari-http://example.com/path?foo=1");
    expect(getEscapeUrl(HTTP_URL, IOS_17_UA)).toBe("x-safari-http://example.com/path?foo=1");
    expect(getEscapeUrl(HTTP_URL, IOS_18_UA)).toBe("x-safari-http://example.com/path?foo=1");
  });

  it("returns null when not in-app", () => {
    expect(getEscapeUrl(HTTPS_URL, "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) Safari")).toBeNull();
  });
